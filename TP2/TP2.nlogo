...
to wolfs-action
  ask wolfs
  [
    if random-float 1000 < random-birth-wolfs and energy > 10
    [
      hatch-wolfs 1
      [
       set color red
...
to wolfs-move
  let food one-of cows in-radius 10
  ifelse food != nobody
  [
...
to wolfs-eat
  let food one-of other cows-here
  if food != nobody
  [
    ask food [ die ]
...
to cows-action
  ask cows
  [
    if random-float 1000 < random-birth-cows and energy > 10
    [
      hatch-cows 1
      [
        set color white
...
to cows-move
  let bull one-of bulls in-radius 5
  ifelse bull != nobody
  [
    set heading towards bull
  ]
  [
    rt random 50
    lt random 50
...
;to update-plot
 ...
 ; plot sum [grass-heigth] of patches  
;end
